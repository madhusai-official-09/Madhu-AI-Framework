...
const advanceFormation = (
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    state.velocity[i] =
      (state.velocity[i] - response * momentum * elapsed) * decay;
  }
  if (
    state.weights.every(
      (value, i) =>
        Math.abs(value - goal[i]) < 0.0001 &&
        Math.abs(state.velocity[i]) < 0.001,
    )
  ) {
    state.weights = goal;
    state.velocity.fill(0);
...
const advanceHold = (hold: HoldState, elapsed: number, disabled: boolean) => {
  ...
  const step =
    engaging && previousElapsed < 0.15 ? hold.elapsed - 0.15 : elapsed;
  ...
  if (
    Math.abs(hold.amount - target) < 0.0001 &&
    Math.abs(hold.velocity) < 0.001
  ) {
    hold.amount = target;
    hold.velocity = 0;
  }
...
const advancePointer = (pointer: PointerState, elapsed: number): void => {
  ...
  for (let axis = 0; axis < 2; axis += 1) {
    ...
    pointer.position[axis] =
      pointer.raw[axis] + (offset + momentum * elapsed) * decay;
  ...
  const presenceMomentum =
    pointer.presenceVelocity + presenceResponse * presenceOffset;
  const nextPresence =
    presenceTarget +
    (presenceOffset + presenceMomentum * elapsed) * presenceDecay;
...
const startRipple = (
  ...
  ripple.duration =
    Math.hypot(farthestX, farthestY) / RIPPLE_SPEED + RIPPLE_TAIL;
  ...
  ASCII_SAMPLES.map(([cx, cy]) => {
    ...
    for (let y = 0; y < 28; y += 1) {
      for (let x = 0; x < 20; x += 1) {
        if (
          ((x + 0.5) / 20 - cx) ** 2 * 0.36 + ((y + 0.5) / 28 - cy) ** 2 >
          0.26 ** 2
        )
          continue;
...
const configureStyle = (
  ...
  const cellWidth =
    ((mode === EFFECTS.ascii ? 3.6 : 1) * outputSize[0]) /
    Math.max(cssSize[0], 1);
  const cellHeight =
    ((mode === EFFECTS.ascii ? 6 : 1) * outputSize[1]) /
    Math.max(cssSize[1], 1);
  ...
  useEffect(() => {
    ...
    visibilityObserver = new IntersectionObserver(
      (entries) => {
        ...
        visible = entry
          ? entry.isIntersecting && visibilityRatio >= 0.02
          : true;
        if (visible) {
          resumePending = true;
        } else {
    ...
    void (async () => {
      try {
        ...
        const preset =
          QUALITY_PRESETS[resolvedQuality] || QUALITY_PRESETS.medium;
        ...
        let grainTime = 0;
        let firstFrame = true;
        const placementMotion = createFormation(
          settingsRef.current!.composition,
        );
        ...
        const renderFrame = (currentFrame: Frame) => {
          ...
          const elapsed =
            resumePending || !previousRenderTimestamp
              ...
              : Math.min(
                  ...
                  Math.max(
                    0,
                    (renderTimestamp - previousRenderTimestamp) / 1000,
                  ),
                );
          resumePending = false;
          ...
          if (frozen) {
            ...
            renderScale +=
              (settings.scale - renderScale) * (1 - Math.exp(-elapsed * 12));
          }
          ...
          const activeCount = Math.max(
            700,
            Math.round(
              preset.count * settings.detailCount * runtimeQuality.countScale,
            ),
          );
          const densityCompensation = Math.pow(
            1 / runtimeQuality.countScale,
            0.2,
          );
          const shardWorldSize =
            0.0125 * settings.shardSize * densityCompensation;
          const aspect = output.size[0] / Math.max(output.size[1], 1);
          const lightPresence =
            settings.interaction === INTERACTIONS.none ? 0 : pointer.presence;
          const pointerShiftX =
            (pointer.position[0] - 0.5) * 0.38 * lightPresence;
          const pointerShiftY =
            (pointer.position[1] - 0.5) * -0.24 * lightPresence;
          ...
          const interactionSign =
            settings.interaction === INTERACTIONS.attract ? 1 : -1;
          ...
          const pointerWorldX =
            ((pointer.position[0] * 2 - 1) * aspect) / renderScale;
          ...
          if (!postEnabled) {
            currentFrame.pass(
              { target: output, clear: settings.background },
              (pass) => {
        ...
        const scheduleFrame = (timestamp: number) => {
          ...
          if (previousRafTimestamp) {
            ...
            if (refreshSample > 3 && refreshSample < 35) {
              ...
              refreshSampleIndex =
                (refreshSampleIndex + 1) % refreshSamples.length;
